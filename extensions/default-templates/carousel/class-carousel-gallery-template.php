	...
	class FooGallery_Carousel_Gallery_Template {
		...
		function admin_custom_fields( $field, $gallery, $template ) {
			if ( isset( $field ) && is_array( $field ) && isset( $field['type'] ) && 'carousel_gutter' === $field['type'] ) {
				...
				$meta_settings = esc_attr( FOOGALLERY_META_SETTINGS );
		
				echo '<label for="FooGallerySettings_' . esc_attr( $id ) . '_min">' . esc_html__( 'Min', 'foogallery' ) . '</label>&nbsp;';
				echo '<input class="small-text" type="number" step="1" min="-1000" id="FooGallerySettings_' . esc_attr( $id ) . '_min" name="' . esc_attr( $meta_settings ) . '[' . esc_attr( $id ) . '][min]" value="' . esc_attr( $min ) . '" />';
				echo '&nbsp;&nbsp;<label for="FooGallerySettings_' . esc_attr( $id ) . '_max">' . esc_html__( 'Max', 'foogallery' ) . '</label>&nbsp;';
				echo '<input class="small-text" type="number" step="1" min="-1000" id="FooGallerySettings_' . esc_attr( $id ) . '_max" name="' . esc_attr( $meta_settings ) . '[' . esc_attr( $id ) . '][max]" value="' . esc_attr( $max ) . '" />';
				echo '&nbsp;&nbsp;<label for="FooGallerySettings_' . esc_attr( $id ) . '_units">' . esc_html__( 'Units', 'foogallery' ) . '</label>&nbsp;';
				echo '<select id="FooGallerySettings_' . esc_attr( $id ) . '_units" name="' . esc_attr( $meta_settings ) . '[' . esc_attr( $id ) . '][units]">';
				echo '<option ' . selected( $units, '%', false ) . 'value="%">' . esc_html__( '%', 'foogallery' ) . '</option>';
				echo '<option ' . selected( $units, 'px', false ) . 'value="px">' . esc_html__( 'px', 'foogallery' ) . '</option>';
				echo '</select>';
				?>
