...
<div id="foogallery-album-<?php echo esc_attr( $current_foogallery_album->ID ); ?>" class="foogallery-container foogallery-stack-album" data-foogallery="<?php echo esc_attr( $data_options ); ?>">
    ...
    <div class="fg-piles">
        ...
        foreach ( $current_foogallery_album->galleries() as $gallery ) {
            ...
            foogallery_album_all_in_one_stack_render_gallery_attachment2( $gallery, $featured_attachment, $args );

            foreach ( $gallery->attachments() as $attachment ) {
                if ( $featured_attachment->ID !== $attachment->ID ) {
                    // Render all but the featured attachment
                    foogallery_album_all_in_one_stack_render_gallery_attachment2( $gallery, $attachment, $args );
                }
            }
            ?></ul><?php
        }
