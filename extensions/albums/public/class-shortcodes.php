    ...
    class FooGallery_Album_Shortcodes {
        ...
        function render_custom_css( $foogallery_album ) {
            if ( !empty( $foogallery_album->custom_css ) ) {
                // Sanitize and output the CSS
                $custom_css = sanitize_textarea_field( $foogallery_album->custom_css ); // Sanitize the input CSS
                echo '<style type="text/css">';
                echo esc_html( $custom_css ); // Escape the sanitized CSS
                echo '</style>';
            }
        }
    }
