		...
		function ajax_foogallery_video_discount_offer() {
			if ( check_admin_referer( 'foogallery_video_discount_offer' ) ) {
				...
				if ( empty( $license_key ) ) {
					...
					echo '<h4>' . sprintf( esc_html__( 'There is no FooVideo license key set for this site. Please set it via the %s under the extensions tab and try again.', 'foogallery' ), $settings_link ) . '</h4>';
				} else {
					...
					if( ! is_wp_error( $response ) ) {
						...
						if ( $response['response']['code'] == 200 ) {
							...
							if ( isset( $license_details ) ) {
								...
								if ( $coupon['valid'] ) {
									echo '<h3>' . esc_html__( 'Your discount code is : ', 'foogallery' ) . $coupon['code'] . '</h3>';
									echo '<h4>' . esc_html__( 'The value of the discount is : ', 'foogallery' ) . $coupon['value'] . '</h4>';
									...
									if ( !class_exists( 'FooGallery_Pro_Video' ) ) {
										echo sprintf( esc_html__( 'Your discount entitles you to a FooGallery PRO - %s license for no additional cost!', 'foogallery' ), $license_option );
										echo '<br />' . sprintf( esc_html__( 'Copy the discount code above and use it when purchasing FooGallery PRO from %s (make sure to select %s plan!).', 'foogallery' ), $pricing_page_link, $license_option );
									} else {
										echo sprintf( esc_html__( 'Your discount entitles you to a free FooGallery PRO - %s license renewal or extension!', 'foogallery' ), $license_option );
										echo '<br />' . sprintf( esc_html__( 'Copy the discount code above and use it when extending your FooGallery PRO license from %s (make sure to select the %s plan!).', 'foogallery' ), $pricing_page_link, $license_option );
									}
