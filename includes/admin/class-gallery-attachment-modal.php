...
if ( ! class_exists( 'FooGallery_Admin_Gallery_Attachment_Modal' ) ) {
	...
	class FooGallery_Admin_Gallery_Attachment_Modal {
		...
		public function display_tab_content_thumbnails( $modal_data ) {
			if ( is_array( $modal_data ) && !empty ( $modal_data ) ) {
				if ( $modal_data['img_id'] > 0 ) {
					...
					$wp_thumbnails = count( wp_get_registered_image_subsizes() );

					// Get FooGallery cache directory for the specific attachment
					$foogallery_thumb_generator = new FooGallery_Thumb_Generator( $modal_data['img_path']);
					...
					<section id="foogallery-panel-thumbnails" class="tab-panel">
						<div class="settings">
							...
							<span class="setting" data-setting="generated-thumbnails">
								<label class="name"><?php _e( 'Generated Thumbnails', 'foogallery' ); ?></label>
								<span><?php echo esc_html( $wp_thumbnails ) . ' ' . esc_html__( 'generated by WP;', 'foogallery' ) . ' ' . esc_html( $foogallery_thumbnails ) . ' ' . esc_html__( 'generated by FooGallery;', 'foogallery' ); ?> </span>												
							</span>
		...
		{
			$wp_filesystem = foogallery_user_uploads_get_wp_filesystem();
