...
if ( isset( $_POST['moderate_image'] ) ) {
	...
	$moderation_action = sanitize_text_field( $_POST['action'] );

	if ( 'approve' === $moderation_action ) {
		...
		if ( $gallery_id && $file_name ) {
			...
			function merge_attachments_with_uploaded_images( $gallery_id, $approved_image, $original_folder, $approved_folder, $metadata_file, $new_metadata_file ) {
				...
				echo '<div class="notice notice-success"><p>' . esc_html__( 'Image approved and added to the gallery successfully.', 'foogallery' ) . '</p></div>';
			}
	...
	} elseif ( 'reject' === $moderation_action ) {
		// Get the gallery ID and file name from the form data.
		$gallery_id = isset( $_POST['gallery_id'] ) ? intval( $_POST['gallery_id'] ) : null;
		$file_name = isset( $_POST['image_id'] ) ? sanitize_text_field( $_POST['image_id'] ) : null;
		...
		if ( $gallery_id && $file_name ) {
			...
			if ( $deleted ) {
				...
				echo '<div class="notice notice-success"><p>' . esc_html__( 'Image successfully rejected', 'foogallery' ) . '</p></div>';
			}
		}
	}
}
...
if ( isset( $_POST['moderate_image'] ) && 'delete' === sanitize_text_field( $_POST['action'] ) ) {
	$image_id    = sanitize_text_field( $_POST['image_id'] );
	$gallery_id  = isset( $_POST['gallery_id'] ) ? intval( $_POST['gallery_id'] ) : null;
	...
	if ( $gallery_id && $image_id ) {
		...
		if ( $deleted ) {
			...
			// Show a success message.
			echo '<div class="notice notice-success"><p>' . esc_html__( 'Image successfully deleted', 'foogallery' ) . '</p></div>';
		}
	}
}
...
if ( is_dir( $user_uploads_dir ) ) {
	...
	foreach ( $directories as $directory ) {
		...
		if ( file_exists( $metadata_file ) ) {
			$metadata_response = wp_remote_get( $metadata_file );
			...
			if ( ! is_wp_error( $metadata_response ) && wp_remote_retrieve_response_code( $metadata_response ) === 200 ) {
				...
				if ( is_array( $metadata ) && isset( $metadata['items'] ) ) {
...
<div class="wrap" id="image-moderation-container">
	...
	<div id="pending-tab" class="tab-content">
		<table class="wp-list-table widefat fixed striped">
			...
			<tbody>
			<?php
			// Check if $images_to_moderate is defined and not null.
			if ( isset( $images_to_moderate ) && is_array( $images_to_moderate ) ) {
				foreach ( $images_to_moderate as $gallery_id => $images ) {
					if ( 0 === $filter_gallery_id || $gallery_id === $filter_gallery_id ) {
