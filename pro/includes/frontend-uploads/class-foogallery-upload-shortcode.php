						...
						<div class="foogallery-upload-popup-content">
							...
							<div class="foogallery-upload-popup-inner">
								...
								<div class="foogallery-upload-right-column">
									<div id="metadata-container" style="padding: 5px 7px;"
									<?php
				...
				if ( isset( $_FILES['foogallery_images'] ) ) {
					...
					foreach ( $uploaded_files['name'] as $key => $filename ) {
						...
						if ( $uploaded_files['type'][ $key ] && strpos( $uploaded_files['type'][ $key ], 'image/' ) === 0 ) {
							...
							if ( move_uploaded_file( $uploaded_files['tmp_name'][ $key ], $user_file ) ) {
								...
								echo '<div class="error-message" style="color: red; text-align: center;">' . esc_html__( 'Error moving the file(s).', 'foogallery' ) . '</div>';
							}
						} else {
							echo '<div class="error-message" style="color: red; text-align: center;">' . esc_html__( 'File is not an image.', 'foogallery' ) . '</div>';
						}
					}
					...
					if ( $exceeded_size_count > 0 ) {
						if ( $exceeded_size_count > 1 ) {
							echo '<div class="error-message" style="color: red; text-align: center;">' . esc_html( $exceeded_size_count ) . ' ' . esc_html__( 'images exceeded the maximum allowed size of ' . $max_image_size . ' MB and were not uploaded.', 'foogallery' ) . '</div>';

						} elseif ( 1 === $exceeded_size_count ) {
							echo '<div class="error-message" style="color: red; text-align: center;">' . esc_html( $exceeded_size_count ) . ' ' . esc_html__( 'image exceeded the maximum allowed size of ' . $max_image_size . ' MB and was not uploaded.', 'foogallery' ) . '</div>';
						}
					}

					// Display success message only if at least one image meets the requirement.
					if ( $uploaded_image_count > $exceeded_size_count ) {
						echo '<div class="success-message" style="color: green; text-align: center;">' . esc_html__( 'Image(s) successfully uploaded and awaiting moderation.', 'foogallery' ) . '</div>';
					}
				} else {
					echo '<div class="error-message" style="color: red; text-align: center;">' . esc_html__( 'No files uploaded or an error occurred.', 'foogallery' ) . '</div>';
				}
			}
		}
