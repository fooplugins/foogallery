	...
	class FooGallery_Pro_Datasource_RealMediaLibrary {
		...
		function render_datasource_modal_content( $foogallery_id, $datasource_value ) {
			...
			if ( ! defined( 'RML_VERSION' ) ) { ?>
				...
				<p><?php printf( esc_html__( 'You need to activate the %s plugin in order to use the Real Media Library datasource', 'foogallery' ), wp_kses_post( $rml_link ) ); ?></p>
				<p><?php esc_html_e( 'RML (Real Media Library) is one of the most wanted media wordpress plugins. It is easy to use and it allows you to organize your thousands of images in folders. It is similar to wordpress categories like in the posts.', 'foogallery' ); ?></p>
				<a href="<?php echo esc_url( $rml_url ); ?>" target="_blank" rel="noopener"><img src="https://ps.w.org/real-media-library-lite/assets/banner-772x250.png" width="772" alt="Real Media Library" /></a>
			<?php } else {

				if ( ! $this->correct_version() ) { ?>
					<p><?php printf( esc_html__( 'You are using an outdated version of %s - please download the latest version of the plugin, which is 100%% datasource compatible.', 'foogallery' ), wp_kses_post( $rml_link ) ); ?></p>
				<?php }
				...
				// Dropdown
				// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- output contains complex markup generated by Real Media Library plugin
