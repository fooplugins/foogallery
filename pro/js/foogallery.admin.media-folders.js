	...
	var FolderTreeView = wp.media.View.extend({
		...
		render: function () {
			...
			var headerHtml = '' +
				...
				'<button type="button" class="foogallery-folder-help-toggle" aria-expanded="false" aria-controls="foogallery-folder-help"><span class="dashicons dashicons-editor-help" aria-hidden="true"></span><span class="screen-reader-text">' + settings.strings.toggleHelp + '</span></button>' +
		...
		moveFolder: function (folderId, newParentId, $rowSpinner) {
			this.setStatus(settings.strings.moving);
			MediaFolders.apiRequest({
				...
				this.termLookup = this.buildTermLookup(this.terms);
				this.render();
				this.setStatus(settings.strings.movingDone);
			}.bind(this)).fail(function () {
				this.setStatus(settings.strings.movingFail);
		...
		reorderSiblings: function (parentId, movingId, targetId, position, $rowSpinner) {
			...
			var self = this;
			this.setStatus(settings.strings.reordering);
			$.post(settings.ajaxUrl, {
				...
				self.render();
				self.setStatus(settings.strings.reordered);
			}).fail(function () {
				self.setStatus(settings.strings.reorderFail);
			}).always(function () {
				MediaFolders.setDraggedFolder(0);
				MediaFolders.setDraggedFolderParent(0);
				if ($rowSpinner) {
		...
		deleteFolder: function (event) {
			...
			if (!window.confirm(settings.strings.deleteConfirm)) {
				return;
			}
		...
		renameFolder: function (folderId, newName) {
			...
			if (!folderId || !newName) {
				this.setStatus(settings.strings.nameRequired);
				return;
			}

			this.setStatus(settings.strings.saving);
			MediaFolders.apiRequest({
				path: '/wp/v2/' + settings.taxonomy + '/' + folderId,
				method: 'POST',
				data: { name: newName },
				...
				this.editingId = null;
				this.render();
				this.setStatus(settings.strings.created);
			}.bind(this)).fail(function () {
				this.setStatus(settings.strings.createFail);
			}.bind(this));
		},
		...
		removeFolder: function (folderId) {
			this.setStatus(settings.strings.deleting);
			MediaFolders.apiRequest({
				...
				this.render();
				this.setStatus(settings.strings.deleted);
			}.bind(this)).fail(function () {
				this.setStatus(settings.strings.deleteFail);
			}.bind(this));
		},
		...
		assignToFolder: function (ids, folderId, $rowSpinner) {
			...
			$status.text(isUnassign ? settings.strings.unassigning : settings.strings.assigning);
