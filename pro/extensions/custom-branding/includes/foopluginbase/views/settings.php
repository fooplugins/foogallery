		...
		<div id="<?php echo $plugin_slug; ?>-settings-main" class="postbox-container">
			<form action="options.php" method="post">
				...
				<p class="submit">
					...
					<input class="button-secondary <?php echo $plugin_slug; ?>-import-export" type="button"
						   value="<?php _e( 'Import / Export Settings', $plugin_slug ); ?>"/>
					<?php do_action( $plugin_slug . '_admin_settings_buttons' ) ?>
				</p>
			</form>
			<div style="display: none" id="<?php echo $plugin_slug; ?>-import-export">
				<div class="metabox-holder">
					<div class="postbox">
						<h3><span><?php _e( 'Export Settings', $plugin_slug ); ?></span></h3>

						<div class="inside">
							<p><?php _e( 'Export the plugin settings for this site as a .json file. This allows you to easily import the configuration into another site.', $plugin_slug ); ?></p>

							<form method="post">
								<p><input type="hidden" name="<?php echo $plugin_slug; ?>_action" value="export_settings"/></p>
								<p>
									<?php wp_nonce_field( $plugin_slug . '_export_nonce', $plugin_slug . '_export_nonce' ); ?>
					...
					<div class="postbox">
						<h3><span><?php _e( 'Import Settings', $plugin_slug ); ?></span></h3>

						<div class="inside">
							<p><?php _e( 'Import the plugin settings from a .json file. This file can be obtained by exporting the settings on another site using the form above.', $plugin_slug ); ?></p>
							...
							<form method="post" enctype="multipart/form-data">
								...
								<p>
									<input type="hidden" name="<?php echo $plugin_slug; ?>_action" value="import_settings"/>
									<?php wp_nonce_field( $plugin_slug . '_import_nonce', $plugin_slug . '_import_nonce' ); ?>
									<?php submit_button( __( 'Import', $plugin_slug ), 'secondary', 'submit', false ); ?>
								</p>
							</form>
						</div>
						<!-- .inside -->
...
<script type="text/javascript">
	jQuery(function($) {
		$('.<?php echo $plugin_slug; ?>-import-export').click(function() {
			$(this).parents('#<?php echo $plugin_slug; ?>-settings').find('#<?php echo $plugin_slug; ?>-import-export').slideToggle();
		});
	});
</script>